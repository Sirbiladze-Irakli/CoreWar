...
/*   Updated: 2020/01/28 16:49:28 by cdubuque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_if_dis_write(t_player *players, int now_pos, int fd)
{
	...
	op = -1;
	write(fd, op_tab[now_pos - 1].name, ft_strlen(op_tab[now_pos - 1].name));
	write(fd, " ", 1);
	if (now_pos != 1 && now_pos != 9 && \
	now_pos != 12 && now_pos != 15)
	{
		while (++op < op_tab[now_pos - 1].col_arg)
		{
			ft_dis_write(players, players->now_pos + 1, fd, op);
			if (op < op_tab[now_pos - 1].col_arg)
				write(fd, " ", 1);
		}
	}
	else
